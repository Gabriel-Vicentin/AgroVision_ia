...
# Em Linux, normalmente 0 aponta para /dev/video0.
# Tambem pode ser um caminho de video, ex: "teste.mp4".
...
def process_stream():
    ...
    if not cap.isOpened():
        print(f"Erro ao abrir camera: {CAMERA_SOURCE}")
        return
...
def get_frame():
    ...
    with last_frame_lock:
        if last_frame is None:
            placeholder = make_status_frame("Sem frame: verifique camera/video.")
            success, buffer = cv2.imencode(".jpg", placeholder)
            if not success:
                return JSONResponse(
                    content={"message": "Erro ao converter frame."}, status_code=500
                )
